...
int sdram_init(void) {
	...
	/* ZCU104: Configure I2C multiplexer before reading SPD */
	printf("Configuring I2C mux for SPD access...\n");
	if (i2c_poll(0x74)) {
		unsigned char mux_data = 0x80;
		if (i2c_write(0x74, 0x00, &mux_data, 1, 1)) {
			printf("I2C mux configured successfully\n");
		} else {
			printf("Failed to write I2C mux channel\n");
		}
	} else {
		printf("I2C mux not found at address 0x74\n");
	}
	
	struct sdram_spd_ctx_s spd_ctx;
	/* Try SPD address 0 first, then 1 for ZCU104 */
	if (sdram_spd_read_i2c(&spd_ctx, 0, 0, 1) == 1) {
		sdram_timings_spd(&spd_ctx);
		printf("SPD: Successfully configured SDRAM timings from SPD (addr 0)\n");
	} else if (sdram_spd_read_i2c(&spd_ctx, 0, 1, 1) == 1) {
		sdram_timings_spd(&spd_ctx);
		printf("SPD: Successfully configured SDRAM timings from SPD (addr 1)\n");
	}
#endif
