...
uint64_t sdram_get_supported_memory(void) {
	...
	/* ZCU104: Configure I2C multiplexer before reading SPD for memory detection */
	if (i2c_poll(0x74)) {
		unsigned char mux_data = 0x80;
		i2c_write(0x74, 0x00, &mux_data, 1, 1);
	}

	if (!sdram_read_spd(0x0, 4, &buf, 1, true)) {
		/* Try SPD address 1 for ZCU104 */
		if (!sdram_read_spd(0x1, 4, &buf, 1, true)) {
