...
def hw_memtest(wb, offset, size, patterns, dbg=False):
    ...
    # 等待读取器变为就绪状态，最多等待5秒
    timeout = 50  # 5秒超时
    while not wb.regs.reader_ready.read() and timeout > 0:
        time.sleep(0.1)
        timeout -= 1
    
    if not wb.regs.reader_ready.read():
        # 尝试更强力的重置 - 恢复BIST读取器的默认配置
        wb.regs.reader_skip_fifo.write(1)
        time.sleep(0.2)
        wb.regs.reader_skip_fifo.write(0)
        
        # 重置关键的BIST配置寄存器到默认状态
        if hasattr(wb.regs, 'reader_modulo'):
            wb.regs.reader_modulo.write(0)
        if hasattr(wb.regs, 'reader_data_div'):
            wb.regs.reader_data_div.write(0)
        if hasattr(wb.regs, 'reader_mem_mask'):
