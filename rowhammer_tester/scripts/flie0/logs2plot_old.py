...
def plot_single_attack(data: dict, _rows: int, cols: int, col_step=32, max_row_cnt=128, title="", save_file=None, show_plot=True, group_rows=64, group_cols=64, use_colorbar=True):
    # Apply custom style if available
    try:
        plt.style.use(PLOT_STYLE)
    except:
        pass  # Fall back to default style if custom style not available
    ...
    if save_file:
        # Ensure high quality output with proper spacing
        plt.tight_layout()
        plt.savefig(save_file, dpi=600, bbox_inches='tight', pad_inches=0.05)
...
def plot_aggressors_vs_victims(data: dict, annotate: str, save_file=None, show_plot=True, group_rows=64, group_cols=64, use_colorbar=True):
    # Apply custom style if available
    try:
        plt.style.use(PLOT_STYLE)
    except:
    ...
    custom_cmap = colors.ListedColormap(["white", *plt.get_cmap("viridis").colors])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save", type=str, help="Save plot to file instead of showing GUI (specify filename)")
    parser.add_argument(
        "--no-show", action="store_true", help="Don't show plot GUI (useful with --save)")
    parser.add_argument(
        "-gr", "--group-rows", type=int, default=64, help="Group rows into N groups (default: 64)")
    parser.add_argument(
        "-gc", "--group-columns", type=int, default=64, help="Group columns into N groups (default: 64)")
    parser.add_argument(
        "--no-colorbar", action="store_true", help="Do not add color bar to plot")
